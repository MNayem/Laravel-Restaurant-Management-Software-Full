...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
  <thead class="header">
  <th colspan="8"><h2 style="text-align: center; color: yellow;">Others DUE LIST</h2></th>
  ...
  <tbody>
     @foreach ($dueshowothers as $dueshowothers)
    <tr align="center">
      <th scope="row">{{ $dueshowothers->id }}</th>
      <td>{{ $dueshowothers->date}}</td>
       <td>{{ $dueshowothers->deliverymanname}}</td>
        <td>{{ $dueshowothers->shopname }}</td>
        <td>{{ $dueshowothers->amount}}</td>
        <td colspan = "3">
            <a href="{{ url('/dueedit',$dueshowothers->id) }}" class="btn btn-primary">Edit </a>
            <a href="{{ url('/deletedue',$dueshowothers->id) }}" class="btn btn-danger ml-2">Delete</a>
        </td>
    </tr>
    @endforeach
  </tbody>
