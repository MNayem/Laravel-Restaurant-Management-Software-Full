...
<div class="container" style="margin-left: 150px;">
   <a href="/jahangirdue" class="btn btn-info">Jahangir Due</a>
...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
  <thead class="header">
  <th colspan="8"><h2 style="text-align: center; color: yellow;">Hridoy DUE LIST</h2></th>
  ...
  <tbody>
     @foreach ($dueshowhridoy as $dueshowhridoy)
    <tr align="center">
      <th scope="row">{{ $dueshowhridoy->id }}</th>
      <td>{{ $dueshowhridoy->date}}</td>
       <td>{{ $dueshowhridoy->deliverymanname}}</td>
        <td>{{ $dueshowhridoy->shopname }}</td>
        <td>{{ $dueshowhridoy->amount}}</td>
        <td colspan = "3">
            <a href="{{ url('/dueedit',$dueshowhridoy->id) }}" class="btn btn-primary">Edit </a>
            <a href="{{ url('/deletedue',$dueshowhridoy->id) }}" class="btn btn-danger ml-2">Delete</a>
        </td>
    </tr>
    @endforeach
  </tbody>
