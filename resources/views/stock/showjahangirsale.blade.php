...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
  <thead class="header">
  <th colspan="27" class="header"><h2 style="text-align: center; color: yellow;" class="header">JAHANGIR SALES TABLE</h2></th>
  ...
  <tbody>
       @foreach ($showjahangirsale as $showjahangirsale)
    <tr align="center">
      <th scope="row">{{ $showjahangirsale->id }}</th>
      <td>{{ $showjahangirsale->pname }}</td>
      <td>{{ $showjahangirsale->date }}</td>
      <td>{{ $showjahangirsale->deliverymanname }}</td>
      <td>{{ $showjahangirsale->packs }}</td>
      <td>{{ $showjahangirsale->pricelp }}</td>
      <td>{{ $showjahangirsale->pricetp }}</td>
      <td>{{ $showjahangirsale->sellingprice }}</td>
      <td>{{ $showjahangirsale->unitprice }}</td>
      <td>{{ $showjahangirsale->orders }}</td>
      <td>{{ $showjahangirsale->cases }}</td>
      <td>{{ $showjahangirsale->pieces }}</td>
      <td>{{ $showjahangirsale->totalpcs }}</td>
      <td>{{ $showjahangirsale->salesv }}</td>
      <td>{{ $showjahangirsale->returnp }}</td>
      <td>{{ $showjahangirsale->returnc }}</td>
      <td>{{ $showjahangirsale->soldc }}</td>
      <td>{{ $showjahangirsale->soldp }}</td>
      <td>{{ $showjahangirsale->tpieces }}</td>
      <td>{{ $showjahangirsale->valuelp }}</td>
      <td>{{ $showjahangirsale->valuetp }}</td>
      <td>{{ $showjahangirsale->damagepcs }}</td>
      <td>{{ $showjahangirsale->damageamount }}</td>
      <td>{{ $showjahangirsale->due }}</td>
      <td colspan="3">
            <a href="{{ url('/dailysaleedit',$showjahangirsale->id) }}" class="btn btn-primary">Edit </a>
            <a href="{{ url('/deletedailysale',$showjahangirsale->id) }}" class="btn btn-danger mt-2">Delete</a>
        </td>
    </tr>
    @endforeach
  </tbody>
