...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
  <thead class="header">
  <th colspan="8"><h2 style="text-align: center; color: yellow;">TOTAL DUE LIST</h2></th>
  ...
  <tbody>
     @foreach ($dueshow as $dueshow)
    <tr align="center">
      <th scope="row">{{ $dueshow->id }}</th>
      <td>{{ $dueshow->date}}</td>
       <td>{{ $dueshow->deliverymanname}}</td>
        <td>{{ $dueshow->shopname }}</td>
        <td>{{ $dueshow->amount}}</td>
        <td colspan = "3">
            <a href="{{ url('/dueedit',$dueshow->id) }}" class="btn btn-primary">Edit </a>
            <a href="{{ url('/deletedue',$dueshow->id) }}" class="btn btn-danger ml-2">Delete</a>
        </td>
    </tr>
    @endforeach
  </tbody>
