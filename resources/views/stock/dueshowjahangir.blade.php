...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
  <thead class="header">
  <th colspan="8"><h2 style="text-align: center; color: yellow;">Jahangir DUE LIST</h2></th>
  ...
  <tbody>
     @foreach ($dueshowjahangir as $dueshowjahangir)
    <tr align="center">
      <th scope="row">{{ $dueshowjahangir->id }}</th>
      <td>{{ $dueshowjahangir->date}}</td>
       <td>{{ $dueshowjahangir->deliverymanname}}</td>
        <td>{{ $dueshowjahangir->shopname }}</td>
        <td>{{ $dueshowjahangir->amount}}</td>
        <td colspan = "3">
            <a href="{{ url('/dueedit',$dueshowjahangir->id) }}" class="btn btn-primary">Edit </a>
            <a href="{{ url('/deletedue',$dueshowjahangir->id) }}" class="btn btn-danger ml-2">Delete</a>
        </td>
    </tr>
    @endforeach
  </tbody>
