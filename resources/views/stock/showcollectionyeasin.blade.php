...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
  <thead>
  <th colspan="10"><h2 style="text-align: center; color: yellow;">YEASIN Collection LIST</h2></th>
  ...
  <tbody>
     @foreach ($showcollectionyeasin as $showcollectionyeasin)
    <tr align="center">
      <th scope="row">{{ $showcollectionyeasin->id }}</th>
       <td>{{ $showcollectionyeasin->date}}</td>
        <td>{{ $showcollectionyeasin->deliveryman }}</td>
        <td>{{ $showcollectionyeasin->shopname }}</td>
        <td>{{ $showcollectionyeasin->dueamount }}</td>
        <td>{{ $showcollectionyeasin->collectionamount }}</td>
        <td>{{ $showcollectionyeasin->amountleft}}</td>
        
        <td colspan = "3">
            <a href="{{ url('/collectionedit',$showcollectionyeasin->id) }}" class="btn btn-primary">Edit </a>
            <a href="{{ url('/deletecollection',$showcollectionyeasin->id) }}" class="btn btn-danger ml-2">Delete</a>
        </td>
    </tr>
    @endforeach
  </tbody>
