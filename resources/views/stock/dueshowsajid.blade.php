...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
  <thead class="header">
  <th colspan="8"><h2 style="text-align: center; color: yellow;">Sajid DUE LIST</h2></th>
  ...
  <tbody>
     @foreach ($dueshowsajid as $dueshowsajid)
    <tr align="center">
      <th scope="row">{{ $dueshowsajid->id }}</th>
      <td>{{ $dueshowsajid->date}}</td>
       <td>{{ $dueshowsajid->deliverymanname}}</td>
        <td>{{ $dueshowsajid->shopname }}</td>
        <td>{{ $dueshowsajid->amount}}</td>
        <td colspan = "3">
            <a href="{{ url('/dueedit',$dueshowsajid->id) }}" class="btn btn-primary">Edit </a>
            <a href="{{ url('/deletedue',$dueshowsajid->id) }}" class="btn btn-danger ml-2">Delete</a>
        </td>
    </tr>
    @endforeach
  </tbody>
