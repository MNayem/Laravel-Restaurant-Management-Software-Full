...
<table class="table table-bordered table-dark" style="margin-left: 160px;">
    ...
    <thead class="header">
        <th colspan="9"><h2 style="text-align: center; color: yellow;">ALL EXPENSE FIELD</h2></th>
    ...
    <tbody>
        @foreach ($expenseshow as $expenseshow)
        <tr align="center">
            <td>{{ $expenseshow->id }}</td>
            <td>{{ $expenseshow->date }}</td>
            <td>{{ $expenseshow->expname }}</td>
            <td>{{ $expenseshow->amount }}</td>
            <td>{{ $expenseshow->deliverymanname }}</td>
            <td>{{ $expenseshow->description }}</td>
            <td colspan = 3>
                <a href="{{ url('/expenseedit',$expenseshow->id) }}" class="btn btn-primary">Edit </a>
                <a href="{{ url('/deleteexpense',$expenseshow->id) }}" class="btn btn-danger">Delete</a>
            </td>
        </tr>
    @endforeach
    </tbody>
